...
void DrawRectangle(uint16_t Xpos,uint16_t Ypos, uint16_t height, uint16_t width){
  ...
  LCD_DrawLine(Xpos,Ypos,Xpos+width,Ypos,Black);
	LCD_DrawLine(Xpos,Ypos,Xpos,Ypos+height,Black); 	
	LCD_DrawLine(Xpos,Ypos+height,Xpos+width,Ypos+height,Black); 	
	LCD_DrawLine(Xpos+width,Ypos,Xpos+width,Ypos+height,Black); 
...
void display_grid(void){
	...
	for(i=0;i<7; i++){
		for(j=0; j<7; j++){
			if(i==0 && j==0){
				DrawSquare(i+offset,j+offset,27);
			}
			else if(i==0 && j!= 0){
